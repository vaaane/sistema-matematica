...
export function renderSidebarAluno(containerId, activePage) {
  ...
  const navHtml  = nav.map(n => `<a class="nav-item${activePage===n.id?' active-orange':''}" href="${n.href}"><svg viewBox="0 0 24 24">${n.icon}</svg>${n.label}</a>`).join('');
...
export function renderSidebarProfessor(containerId, activePage) {
  const nav = [
    ...
    { id:"analise",        href:"/professor/analise.html",        label:"Análise",        icon:`<path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>` },
  ...
  const navExtra = [
    ...
    { id:"acomp_avaliacao", href:"/professor/acompanhamento_avaliacao.html", label:"Acomp. Avaliação+", icon:`<path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>` },
